...
while IFS=',' read -r add_time_ns resolve_time_ns chan_id_in chan_id_out amt_in_msat amt_out_msat fee_msat; do
  # Remove leading/trailing double quotes if present
  add_time_ns="${add_time_ns//\"/}"
  resolve_time_ns="${resolve_time_ns//\"/}"
  ...
  echo "$add_time_ns, $resolve_time_ns, $random_id_in, $random_id_out, $amt_in_msat, $amt_out_msat, $fee_msat" >> "$output_file"
